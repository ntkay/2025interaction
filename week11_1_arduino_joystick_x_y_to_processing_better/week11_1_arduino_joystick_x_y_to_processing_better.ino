//week11_1_arduino_joystick_x_y_to_processing_better
...
//Serial.write(x/4); //把0-1023 變成0-255
//Serial.write(y/4);
  if(abs(x-x0)<25) x = 128; //數值變化大小,直接放中間值128
  else x = (x-x0)/4.4+128; //有大的數值,就滅掉中間值,再除4,再加128
  if(abs(y-y0)<25) y = 128; //數值變化大小,直接放中間值128
  else y = (y-y0)/4.4+128; //有大的數值,就滅掉中間值,再除4,再加128
