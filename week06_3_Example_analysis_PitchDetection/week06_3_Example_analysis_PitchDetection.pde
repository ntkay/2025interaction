...
// 想要把聲音最重要的頻率找出來,使用PitchDetection物件
/**
...
SoundFile sound; //新家這行,因為要把音樂beat.mp3讀進來
...
void setup() {
  ...
  sound = new SoundFile(this, "beat.mp3"); //新加這一行,輸入beat.mp3
  sound.loop();
  pitchDetector = new PitchDetector(this); //新增、初始化Pitchdetection物件
  //pitchDetector.input(new AudioIn(this, 0)); //原本的程式,是用audio in 音效卡的音源輸入
...
void draw() {
  ...
  if (pitchAndConfidence[1] > 0) {
    ...
    println(pitchAndConfidence[0],pitchAndConfidence[1]); //把pitch、confidence都印出來
  }
